...
The app image is built and pushed to `ghcr.io/nirsaban/pakahim-shift-manager`
...
1. On the VPS: `mkdir -p /opt/pakahim && cd /opt/pakahim`, then copy
...
4. Point DNS for `pakahim.miltech.cloud` at the VPS, then obtain a cert
...
7. Seed once, manually — **do not** rely on any automatic seeding:
   ```bash
   docker exec pakahim-app npx tsx prisma/seed.ts
   ```
...
2. `git revert` (or push a previous commit) to `main` — the pipeline
   ...
   ```bash
   docker pull ghcr.io/nirsaban/pakahim-shift-manager:<previous-sha>
   docker tag ghcr.io/nirsaban/pakahim-shift-manager:<previous-sha> ghcr.io/nirsaban/pakahim-shift-manager:latest
